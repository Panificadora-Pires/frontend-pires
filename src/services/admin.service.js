...
const adminService = {
  ...
  alterarStatusPedido(id, status) {
    return api.patch(`/pedidos/${id}/alterar_status/`, { status }).then(({ data }) => data)
  },

  retirarViaQRCode(codigoRetirada) {
    return api.post('/pedidos/retirar_via_qrcode/', {
      codigo_retirada: codigoRetirada,
    }).then(({ data }) => data)
  },

  relatorioVendas({ dataInicio, dataFim }) {
    return api.get('/pedidos/relatorio_vendas/', {
      params: {
  ...
  criarProduto(payload) {
    return api.post('/produtos/', produtoFormData(payload)).then(({ data }) => data)
  },

  atualizarProduto(id, payload) {
    return api.patch(`/produtos/${id}/`, produtoFormData(payload)).then(({ data }) => data)
  },

  maisVendidos({ dataInicio, dataFim, limite = 10 } = {}) {
    return api.get('/produtos/mais_vendidos/', {
      params: {
