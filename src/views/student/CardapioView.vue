...
<style scoped>
.menu-page { width:min(100%,1240px); margin:0 auto; color:var(--student-text); }
.menu-page__header { display:flex; justify-content:space-between; align-items:flex-end; gap:22px; margin-bottom:22px; }
.menu-page__eyebrow { display:none; }
.menu-page__header h1 { margin:0; font-size:30px; font-weight:700; }
.menu-page__header p { margin:6px 0 0; color:var(--student-muted); font-size:13px; }
.menu-page__cart-summary { min-width:188px; height:52px; display:grid; grid-template-columns:36px 1fr 16px; align-items:center; gap:9px; padding:0 12px; border:1px solid var(--student-border); border-radius:8px; background:#fff; color:var(--student-text); }
.menu-page__cart-icon { position:relative; width:34px; height:34px; display:grid; place-items:center; border-radius:7px; background:#f3efe9; color:#6a4a1f; }
.menu-page__cart-badge { position:absolute; top:-6px; right:-6px; min-width:17px; height:17px; display:grid; place-items:center; padding:0 4px; border-radius:999px; background:var(--student-accent); color:#fff; font-size:9px; font-weight:800; }
.menu-page__cart-summary small { display:block; color:var(--student-muted); font-size:9px; }
.menu-page__cart-summary strong { display:block; margin-top:1px; font-size:11px; font-weight:700; }
.menu-page__toolbar { display:grid; grid-template-columns:minmax(0,1fr) 205px; gap:10px; margin-bottom:12px; }
.menu-page__search,.menu-page__sort { height:42px; display:flex; align-items:center; gap:8px; padding:0 12px; border:1px solid var(--student-border); border-radius:8px; background:#fff; color:#867e76; }
.menu-page__search:focus-within { border-color:#c9a466; box-shadow:0 0 0 3px rgba(184,121,31,.07); }
.menu-page__search input { min-width:0; width:100%; border:0; outline:0; background:transparent; color:var(--student-text); font-size:12px; }
.menu-page__search button { display:grid; place-items:center; padding:4px; border:0; background:transparent; color:#8b837a; cursor:pointer; }
.menu-page__sort select { width:100%; appearance:none; border:0; outline:0; background:transparent; color:var(--student-text); font-size:11px; }
.menu-page__categories,.menu-page__quick-filters { display:flex; gap:6px; overflow:auto; scrollbar-width:none; }
.menu-page__categories::-webkit-scrollbar,.menu-page__quick-filters::-webkit-scrollbar{display:none}
.menu-page__categories { margin-bottom:10px; }
.menu-page__categories button,.menu-page__quick-filters button { flex:0 0 auto; min-height:34px; display:flex; align-items:center; gap:6px; padding:0 11px; border:1px solid var(--student-border); border-radius:7px; background:#fff; color:#6f675f; font-size:11px; font-weight:600; cursor:pointer; }
.menu-page__categories button.is-active,.menu-page__quick-filters button.is-active { border-color:#c9a466; background:var(--student-accent-soft); color:#7b5117; }
.menu-page__filter-row { display:flex; justify-content:space-between; gap:10px; margin-bottom:24px; }
.menu-page__clear { flex:0 0 auto; border:0; background:transparent; color:#87591a; font-size:11px; font-weight:700; cursor:pointer; }
.menu-page__content-head { display:flex; align-items:flex-end; justify-content:space-between; gap:16px; margin-bottom:13px; }
.menu-page__content-head h2 { margin:0; font-size:19px; }
.menu-page__content-head p { margin:4px 0 0; color:var(--student-muted); font-size:11px; }
.menu-page__result-badge { min-width:26px; height:26px; display:grid; place-items:center; border-radius:6px; background:#eee9e2; color:#655d55; font-size:10px; font-weight:700; }
.menu-page__grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:13px; }
.menu-page__skeleton { min-height:310px; border:1px solid var(--student-border); border-radius:10px; background:#fff; overflow:hidden; }
.menu-page__skeleton div { height:190px; background:linear-gradient(90deg,#eeeae5,#f7f5f2,#eeeae5); background-size:200% 100%; animation:menu-shimmer 1.2s infinite; }
.menu-page__skeleton span { display:block; height:11px; margin:14px 12px 0; border-radius:4px; background:#eeeae5; }
.menu-page__skeleton span:last-child { width:55%; margin-top:8px; }
.menu-page__state { min-height:230px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:28px; border:1px solid var(--student-border); border-radius:10px; background:#fff; }
.menu-page__state-icon { width:48px; height:48px; display:grid; place-items:center; margin-bottom:12px; border-radius:50%; background:#f3efe9; color:#8f672b; }
.menu-page__state h2 { margin:0; font-size:17px; }.menu-page__state p{margin:6px 0 14px;color:var(--student-muted);font-size:12px}.menu-page__state button{min-height:36px;padding:0 12px;border:1px solid var(--student-border-strong);border-radius:7px;background:#fff;color:var(--student-text);font-size:11px;font-weight:700;cursor:pointer}
.menu-page__toast { position:fixed; right:24px; bottom:24px; z-index:90; display:flex; align-items:center; gap:7px; padding:10px 13px; border:1px solid #cadbcd; border-radius:8px; background:#f4faf5; color:#33583a; box-shadow:0 10px 30px rgba(30,40,30,.1); font-size:11px; font-weight:600; }
.menu-toast-enter-active,.menu-toast-leave-active{transition:opacity .15s ease}.menu-toast-enter-from,.menu-toast-leave-to{opacity:0}@keyframes menu-shimmer{to{background-position:-200% 0}}
@media(max-width:1120px){.menu-page__grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media(max-width:760px){.menu-page__header{align-items:flex-start;flex-direction:column}.menu-page__header h1{font-size:25px}.menu-page__cart-summary{width:100%;min-width:0}.menu-page__toolbar{grid-template-columns:1fr}.menu-page__sort{width:100%}.menu-page__categories,.menu-page__filter-row{margin-left:-18px;margin-right:-18px;padding-left:18px;padding-right:18px}.menu-page__filter-row{display:block}.menu-page__quick-filters{margin-right:-18px;padding-right:18px}.menu-page__clear{margin-top:9px}.menu-page__grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.menu-page__toast{left:13px;right:13px;bottom:78px}}
</style>
