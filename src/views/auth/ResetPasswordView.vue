<template>
  <AuthLayout
    :back-to="{ name: 'recuperar-senha' }"
    back-label="Solicitar outro código"
  >
    <template #heading>
      <h1>Defina uma <span>nova senha</span></h1>
      <p v-if="pendencia">
        Digite o código enviado para {{ pendencia.email }} e escolha sua nova
        senha.
      </p>
      <p v-else>
    ...
    <div v-else class="auth-actions-stack">
      ...
      <BaseButton
        block
        :icon="Mail"
        @click="router.push({ name: 'recuperar-senha' })"
      >
        Solicitar código
      </BaseButton>
    </div>
