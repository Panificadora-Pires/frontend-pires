<template>
  <section class="admin-page">
    <header class="admin-page-header">
      <div>
        ...
        <p class="admin-page-subtitle">
          Organize a estrutura do cardápio e a ordem de exibição das categorias.
        </p>
      </div>
    ...
    <div v-if="!categorias.length" class="admin-card admin-empty">
      <Layers3 :size="28" /><strong>Nenhuma categoria cadastrada</strong>
    </div>
          ...
          <tr v-for="c in categorias" :key="c.id">
            ...
            <td>
              <span
                class="admin-badge"
                :class="
                  c.ativa ? 'admin-badge--success' : 'admin-badge--neutral'
                "
                >{{ c.ativa ? "Ativa" : "Inativa" }}</span
              >
            </td>
            <td class="categories-table__action">
              <button class="admin-btn admin-btn--sm" @click="editar(c)">
      ...
      <form class="admin-modal-card" @submit.prevent="salvar">
        ...
        <div class="admin-modal-body admin-form-grid">
          ...
          <label class="admin-field admin-field--full"
            ...
            ><input v-model.trim="form.descricao"
          /></label>
          <label class="admin-field"
            ><span class="admin-field-label">Ordem de exibição</span
            ><input v-model.number="form.ordem" type="number" min="0"
          /></label>
          <label class="category-check"
            ><input v-model="form.ativa" type="checkbox" /><span
              ><strong>Categoria ativa</strong
              ><small>Permite exibir a categoria no cardápio.</small></span
            ></label
          >
        </div>
